...
// declare const L: LeafletGlobal; (TypeScriptの宣言なので削除)

/**
 * Leafletマップを初期化し、指定されたHTML要素にマウントします。
 * @param {string} mapid - マップをマウントするHTML要素のID。
 * @param {Function} onShapeCreated - ユーザーが図形を描画したときに呼び出されるコールバック関数。
 * @returns {Object} 初期化されたLeafletマップのインスタンス。
 */
export function initMap(mapid, onShapeCreated) {
     ...
     * @param {Object} data - 表示する情報を含むデータオブジェクト。
     * @returns {Object} 作成された矩形レイヤー。
     */
